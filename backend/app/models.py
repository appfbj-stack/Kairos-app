...
class Membro(Base):
    ...
    data_filiacao: Mapped[date | None] = mapped_column(Date, nullable=True)
    email: Mapped[str | None] = mapped_column(String(255), nullable=True)
    ...
    importacao_id: Mapped[str | None] = mapped_column(String(36), ForeignKey("importacoes_log.id"), nullable=True, index=True)
...
class ConhecimentoIA(Base):
    __tablename__ = "assistente_conhecimento"
    id: Mapped[str] = mapped_column(String(36), primary_key=True)
    tenant_id: Mapped[int] = mapped_column(ForeignKey("tenants.id"), index=True)
    titulo: Mapped[str] = mapped_column(String(255))
