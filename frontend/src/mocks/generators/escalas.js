...
import { mulberry32, nomeBR, int, dateBetween, pick, chance } from '../../lib/seed';

const DEPT_IDS = ['D_HOMENS','D_MULHERES','D_JOVENS','D_LOUVOR','D_INTERCESSAO','D_EVANGELISMO','D_MISSOES','D_CRIANCAS'];
const TIPOS_ESCALA = ['louvor','pregação','intercessão','recepcionista','louvor','louvor','pregação','intercessão'];
...
export function gerarEscalas(count = 200, seed = 99999, congregacoesIds = []) {
  ...
  for (let i = 0; i < count; i++) {
    list.push({
      ...
      tipo: pick(TIPOS_ESCALA, rng),
      membro: nomeBR(rng),
      ativa: chance(0.75, rng),
      obreiros: Array.from({ length: int(3, 12, rng) }, () => `O${int(1, 250, rng).toString().padStart(4, '0')}`),
      status: pick(['pendente','confirmada','realizada','cancelada'], rng),
      confirmados: int(0, 12, rng),
      ausencias: int(0, 5, rng),
      trocas: int(0, 3, rng),
    });
  }
  return list;
